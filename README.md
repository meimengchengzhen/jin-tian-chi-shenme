...
- **首次进入会弹出一个轻量「个性化设置」对话框**，可关、可跳过：
  - 弹窗顶部强调 **数据全部保存在本机浏览器里，不上传任何服务器**（沙箱预览环境会退回到内存态，关闭页面即清除）。
  - 三步可选填：① 今天来这儿的目的（一键决定 / 家庭掌勺 / 减脂控卡 / 健康忌口 / 外卖党 / 旅行美食 / 饭桌陪伴）→ ② 心情节奏 + 健康关注 → ③ 性别 / 年龄段 / 身高 / 体重 / 目标体重 / 人数 / 城市 / 预算（全部可跳过）。
  - 选完目的可一键「去推荐入口」：减脂控卡 → 健康饮食 Tab，家庭掌勺 → 一周菜单 Tab，外卖党 → 外卖 Tab，懒人 → 懒人决定 Tab，旅行 → 旅行美食 Tab，陪伴 → 饭桌陪伴 Tab。
  - 顶栏新增「个性化」按钮，随时可以重开弹窗。
...
| 个性化设置（v9）— 首次进入轻量弹窗，全部本地存储 | ![个性化设置](docs/screenshots/persona-welcome.png) |
