...
- **今晚最终方案（`#/tonight-plan`）** — 上面两条任意一条点「就按这个」之后，会沉淀成一份结构化最终方案卡片：聚合菜 / 外卖 / 零食 / 水果 / 看 / 听 / 家庭兼容 / 冰箱缺料 / 剩菜改造 / 明天衔接 / 预算 / 热量 一目了然。提供「复制方案」（带剪贴板兜底）/「再生成」/「继续完善」（健康 / 冰箱 / 剩菜 / 一周菜单等深挖入口）。
- **统一购物清单（`#/shopping`）** — 把「家庭今晚饭缺料 / 今晚最终方案缺料 / 冰箱有啥差几样 / 剩菜改造要补 / 一周菜单买菜清单 / 手动添加」六个来源的食材统一沉淀到一张可勾选清单：按 `肉蛋奶 / 蔬菜 / 水果 / 主食豆制品 / 调味杂项 / 其他` 分类展示，同名食材自动按 `ingredientAliases` 合并（番茄 / 西红柿不会成两行）并保留来源备注；支持勾选 / 删除 / 手动添加 / 一键复制（带剪贴板兜底）/ 清理已买 / 清空全部。点「就按这个」时会把家庭饭缺料一并加入清单，避免还要再点一次。所有数据走 `safeGet/safeSet`，隐私模式自动退回内存态；不接外部买菜平台 API、不做真实价格比价。
...
- **海报式「今晚最终方案」（`#/tonight-plan`）** — 把最终方案做成「适合截图分享」的海报：顶部色系切换（**清爽蓝绿 / 暖橙治愈 / 夜间紫**，单人默认暖橙、家庭默认蓝绿），强化标题 / 日期 / 单人 vs 一家人徽章 / 预算热量摘要 / 鼓励语等视觉层级；卡片化分区，手机端不再过长。保留复制方案 / 再生成 / 继续完善（健康 / 冰箱 / 剩菜 / 一周菜单 / 购物清单）入口。色系选择会通过 `localStorage` 记住，下次进来不变。
- **饭桌陪伴内容池（`#/companion` / `#/solo` / `#/tonight-plan`）** — `WATCH_ITEMS / TOPIC_ITEMS / AUDIO_ITEMS` 总量已扩到 600+（影视 ≥ 200、话题 ≥ 200、音频 ≥ 200），各分类（电影 / 电视剧 / 综艺 / 纪录片 / 动画 / 有声书 / 播客 / 音乐 / 助眠 / 故事）均有较丰富覆盖；`check:recommend` 强制断言「三池都 ≥ 200」。
- **「今日懒人惊喜简餐」内容池（`#/lazy` / `#/solo`）** — `LAZY_MEALS` 扩到 250+，覆盖 5/10 分钟、免开火、便利店组合、外卖平替、宿舍/办公室、减脂、夜宵、早餐、甜品饮品等场景；每条都有名称 / emoji / 设备 / 估时 / 食材 / 目标标签 / 估算热量 / 估算价格 / 步骤 / 替代食材 / 一句话推荐理由。`check:recommend` 现在断言 `LAZY_MEALS.length >= 200`。
- **剩菜变花样（`#/leftover`）扩展** — `LEFTOVER_RULES` 增加红烧肉 / 鸡 / 鱼 / 米饭 / 面 / 火锅 / 饺子 / 烤鸭 / 烧烤 / 粥 / 河粉 / 凉皮 / 年糕 / 寿司 / 三明治 / 披萨 / 咖喱 / 粽子 / 卤味 / 蒸蛋 / 大盘鸡 / 羊肉 等 49 条 rule，**变形方案总数 81 条**；`check:recommend` 断言「变形方案 ≥ 80」+ 兜底方案永不空白。
- **冰箱有啥（`#/fridge`）扩展** — `FRIDGE_PRESETS` 拆为 7 个分组（蛋白质 / 蔬菜 / 主食 / 冷冻 / 调味 / 奶蛋饮品 / 水果），共 128 条常见食材；`ingredientAliases` 增补到 200+ 别名（如三文鱼/巴沙鱼/上海青/小番茄/速冻饺子/紫甘蓝/全麦面包/凉皮/腊肉/午餐肉/希腊酸奶 等），让用户输入 / 添加任意常见叫法都能命中标准名，提升推荐丰富度，购物清单合并也更准。

> 注：以上扩展均为**内置推荐池**（不联网 / 不接外部内容 API），可视作「编辑选送的清单」+ 本地随机刷新，并不声明实时性。
