...
[在线体验](https://meimengchengzhen.github.io/jin-tian-chi-shenme/) · [功能总览](#功能总览) · [为什么做这个](#a为什么做这个解决方案) · [怎么实现的](#b具体怎么实现的) · [启发与发现](#c做完之后的启发与发现) · [本地运行](#本地运行--部署) · [路线图](#路线图) · [创作札记](#创作札记--从选择困难到一点期待)
...
**饭搭子** 面向有「吃饭选择困难」的中文用户：

- 🧍 **一个人** ——「中午晚上不知道吃啥、点外卖刷半小时、零食货架站五分钟还在纠结」
- 👨‍👩‍👧 **一家人** —— 老人要清淡、孩子不吃辣、自己想减脂、伴侣口味又重，每顿饭都众口难调
- 🏠 **独居** —— 一个人做饭怕浪费、点外卖怕重复、冰箱里总有半根胡萝卜不知道怎么处理
- 💪 **想吃得健康** —— 减脂、低糖低盐、控卡、增肌，但不想被复杂的营养软件吓退

它把「今天吃什么 / 买什么菜 / 点什么外卖 / 减脂怎么吃 / 给家人做什么 / 吃饭看什么聊什么」收进同一个 PWA 网页里，纯前端、零登录、刷新就能用。
...
> 一些藏在代码之外、关于「为什么是吃饭，为什么是这种做法」的私人笔记。

### 为什么偏偏选「吃」这件事

每天最稳定地把我卡住的，不是工作，是「中午吃啥、晚上吃啥」。
打开外卖 App，先比价、再凑券、再算配送费，刷十分钟还没下单；点开视频网站想配饭，又要在「今天看什么剧」里再纠结一次。
问了身边几个朋友、看了一圈社交平台，发现这不是我一个人的怪癖：

- **单人用户**：选项越多越累，想要的是「替我先选一个」。
- **家庭用户**：要同时照顾老人清淡、孩子不吃辣、自己想减脂、伴侣口味重，还得算着预算逛菜场。
- **独居用户**：怕浪费、怕重复、冰箱里永远有半根胡萝卜。
- **想吃得健康的人**：被各种 App 的卡路里输入、营养面板劝退，只想要一句「这顿能不能吃」。

这些麻烦看起来零碎，加起来却是每天都在被切走的注意力。

### 这不是一个「随机转盘」

最早我也以为做个「点一下随机出一道菜」的小工具就够了，但试用几次就明白：随机不解决问题，**约束**才是问题——预算、人数、心情、健康目标、冰箱里有什么、昨天刚吃过什么……
所以饭搭子的本质是把围绕「吃」的几条主线拧到一起：

- **菜谱 + 买菜清单**：决定吃什么的同时，把买什么、按类目怎么买列好。
- **外卖 + 零食 + 水果**：实在不想做饭那一档，也有备选；不走商家直链，全部用稳定搜索入口，不依赖任何商家 API。
- **饭桌陪伴**：吃饭看什么 / 聊什么 / 听什么，省得再开第二个 App 纠结。
- **家庭 / 冰箱 / 剩菜**：把一家人怎么协调、冰箱里能凑出什么、昨天剩的怎么变花样，串成一条完整动线。
- **健康规划**：BMR/TDEE、低糖低盐、控卡偏好做软排序——只做参考，不替医生说话。

希望用「推荐 + 个性化」减少跨平台搜索、信息焦虑、被推送广告劝退的那种隐性消耗。

### 怎么做出来的：和多个模型协作

这个项目不是一个人在 IDE 里硬写出来的，而是**和多个模型/代码代理一起做**的：

- 主要思路、架构方案、功能取舍：在 Perplexity Computer 里通过对话一点点掰扯清楚，连接 GitHub 仓库后由它统一调度。
- 仓库内的具体实现、bug 修复、回归测试、方向分析：交给 Claude Code 等子代理在分支上并行推进——读代码、改代码、跑 `check:recommend / check:links`、写 PR、回报问题。
- 有时是「同一个时刻多个代理在不同模块独立工作」，有时是「一个代理跑完测试，另一个接手做改造」。

人去定问题、定边界、定品味；代理去把活干掉，再把不确定的地方抛回来让人决定。**很多功能能在一周内成型，不是因为我变快了，而是因为我变得更像一个产品负责人。**

### 卡过的弯路：从「实时外卖」转向「可落地」

最早设想里有一条很激进的线：**实时拉取附近商家、当前优惠、配送费，给出此时此刻的最优解**。
真做的时候发现：

- 各平台的实时数据、优惠规则、配送时段不会开放给一个独立网站。
- 能拿到的东西要么需要登录态，要么是商家页 H5（链接易变、还容易封）。
- 即使能爬到，也无法对结果负责——价格 1 分钟一变，承诺得太重就是骗用户。

于是把方向收回来：

- 外卖只做「连锁品牌 + 稳定搜索入口」，再用本地排序刷新；
- 推荐核心放在「菜谱 + 买菜 + 健康偏好 + 家庭 / 冰箱 / 剩菜」这些**自己能掌控的静态数据 + 规则**上；
- 该承认的局限明确写清楚：不实时、不抓商业 API、健康仅作参考。

少做一点，但让做出来的东西站得住。

### 做完之后冒出来的一点想法

- 日常里有大量「微小决定」——午饭、配饭看的剧、要不要点奶茶、晚餐配菜——这些决定单个不大，但一天下来真的会累。
- 如果一个工具能把「今天吃什么」从压力变成「今天会吃到什么」的小期待，它的价值就不只是功能型工具，更接近一种**生活节奏的轻调度**。
- 未来如果能在用户授权下，跟外卖 / 买菜平台做更深的合作，把「替你决定 → 替你下单 → 给你一个饭点小盲盒」走通，会是一件挺有意思的事。
- 但在那之前，先把「不联网也能给出靠谱建议」做扎实——这才是这个工具值得长期存在的底盘。
